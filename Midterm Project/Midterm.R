...
MidtermData$AtRisk <- as.factor(ifelse(MidtermData$X1st_yr_GPA <= 1.7, "At Academic Risk", "Not At Academic Risk"))
MidTermDataTrain$AtRisk <- as.factor(ifelse(MidTermDataTrain$X1st_yr_GPA <= 1.7, "At Academic Risk", "Not At Academic Risk"))
MidTermDataTest$AtRisk <- as.factor(ifelse(MidTermDataTest$X1st_yr_GPA <= 1.7, "At Academic Risk", "Not At Academic Risk"))
...
M_TrainAR <- glm(formula = AtRisk ~ HSGPA + Housing + ERG8 + Stem + ERG5 + East + ERG1 + FGEN, family = binomial, data = MidTermDataTrain)
summary(M_TrainAR)

M_Train2AR <- glm(formula = AtRisk ~ HSGPA + Housing + ERG8 + Stem + ERG5 + East, family = binomial, data = MidTermDataTrain)
summary(M_Train2AR)

M_Train3AR <- glm(formula = AtRisk ~ HSGPA + Housing + ERG8 + Stem + East, family = binomial, data = MidTermDataTrain)
summary(M_Train3AR)
1-with(M_Train3AR,deviance/null.deviance)
...
######ACCURACY USING TABLES + ROCR##########
step_default <- stepAIC(M_Train3AR, direction = "backward")
summary(step_default)

step_default_pred = ifelse(predict(step_default,type = "response") >.5, "yes","no")
head(step_default_pred)
Table_train <- table(predicted = step_default_pred, actual = MidTermDataTrain$trainortest)

pred <- predict(M_Train3AR, type = "response")
predObj <- prediction(pred, )
